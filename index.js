...
var require_http_chain_client = __commonJS({
  "node_modules/drand-client/http-chain-client.js"(exports) {
    ...
    exports.default = HttpChainClient3;
  }
});
...
var require_beacon_verification = __commonJS({
  "node_modules/drand-client/beacon-verification.js"(exports) {
    ...
    var bls = __importStar(require_lib());
    var bls12_381_1 = require_lib();
    var index_1 = require_drand_client();
...
var require_drand_client = __commonJS({
  "node_modules/drand-client/index.js"(exports) {
    ...
    async function fetchBeacon2(client, roundNumber) {
      let beacon = null;
      if (!roundNumber) {
...
function md5(bytes2) {
  ...
  return import_crypto3.default.createHash("md5").update(bytes2).digest();
}
...
function sha1(bytes2) {
  ...
  return import_crypto4.default.createHash("sha1").update(bytes2).digest();
}
...
// src/select.ts
async function select(count, values, beacon) {
...
async function parseGithubOptions() {
  ...
  const inputFiles = await import_promises.default.readdir(import_path.default.join(gitRepo, inputDir));
  const outputFiles = await import_promises.default.readdir(import_path.default.join(gitRepo, outputDir));
...
async function readDrawItems(inputFile, gitRepo, inputDir) {
  console.log(`processing ${inputFile}`);
  const contents = await (0, import_promises2.readFile)(path2.join(gitRepo, inputDir, inputFile));
  return contents.toString().split("\n").filter((it) => it.trim() !== "");
}
async function drawWithDrand(drandClient, count, values) {
  const chainInfo = await drandClient.chain().info();
  const nextRound = (0, import_drand_client2.roundAt)(Date.now(), chainInfo) + 1;
  const beacon = await retry(35, () => (0, import_drand_client2.fetchBeacon)(drandClient, nextRound));
  return select(count, values, beacon);
}
async function writeSuccessfulDraw(name, drawItems, selectionOutput, gitRepo, outputDir, outputFilename) {
  const fileOutput = {
    time: Date.now(),
    name,
  ...
  await fs2.writeFile(path2.join(gitRepo, outputDir, outputFilename), JSON.stringify(fileOutput));
